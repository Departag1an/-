...
void solve() {
    ...
    // c[0]个长为n/k的子数组
    // c[1]个长为n/k+1的子数组
    vector<vector<ll>> f(k + 1, vector<ll>(c[1] + 1, inf2));
    // f[i][j] 前i个子段当中有j个是+1长度的子段的极差和
    f[0][0] = 0;
    for (int i = 1; i <= k; i ++) {
        for (int j = 0; j <= c[1]; j ++) {
            // 最后一段取不+1长度, 末尾元素是nums[n / k * i + j - 1]
            f[i][j] = min(f[i][j], f[i - 1][j] + a[n / k * i + j - 1] - a[n / k * (i - 1) + j]);
            // 最后一段取+1长度
            if (j > 0) {
                f[i][j] = min(f[i][j], f[i - 1][j - 1] + a[n / k * i + j - 1] - a[n / k * (i - 1) + j - 1]);
            }
        }
    }
    cout << f[k][c[1]] << endl;
