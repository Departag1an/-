...
#define FAST ios::sync_with_stdio(false);cin.tie(0),cout.tie(0);cout<<fixed<<setprecision(12)
...
void solve() {
    ...
    auto check = [&](double t) {
        double l = -1e9, r = 1e9;
        for (int i = 0; i < n; i ++) {
            double lower = x[i] - v[i] * t, upper = x[i] + v[i] * t;
            if (lower > r or upper < l) return false;
            l = max(l, lower);
            r = min(r, upper);
        }
        return true;
