...
bool check(map<int, int>& mp, int x) {
    for (int i = 2; i <= x / i; i ++) {
        if (x % i == 0) {
            ...
            int c = 0;
            while (x % i == 0) c ++, x /= i;
            if (c > mp[i]) return false;
        }
    }
    if (x > 1 and !mp.count(x)) return false;
    return true;
}
void solve() {
    cin >> n >> q;
    ...
    while (q --) {
        ...
        if (op == 1) {
            ...
            cin >> x;
            get(mp, x);
            // a m 互质 必然不在mp中 因此需要mp中凑出m能整除的个数
            int d = cald(mp);
            cout << (check(mp, d) ? "YES" : "NO") << endl;
        }
        else {
            mp.clear();
            get(mp, n);
